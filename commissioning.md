...
| C-23 | **HAZARD ROW, not a measurement. THE C-02 WINDOW MUST SIT INSIDE A RUNNING PERIOD OF THE CIRCULATION PUMP AND MUST NOT SPAN A START OR A STOP, AND NOTHING IN THE SYSTEM ENFORCES THAT. THE PERSON HOLDING THE STOPWATCH IS THE ONLY ENFORCEMENT.** Look at the pump, confirm it is running, confirm it will still be running at the end of the window, and only then start the window. If it starts or stops mid-window, **the run is DISCARDED, not adjusted** | D-063 and F-070. Circulation is INTERMITTENT: it runs during a batch, during dosing and through a settle window, and is off between batches. A window spanning a transition measures two mixing regimes and averages them into one number that describes neither. **Software cannot catch this: under G-26 and D-052 the Pi neither commands nor observes the circulation pump, so it cannot know a transition happened.** software-spec.md section 6.4 states this as a procedural condition on the operator and NOT ENFORCEABLE IN SOFTWARE. **It is written here as well as there because the person it constrains will never read the software spec** | Owner | **Applies to every row that reads the tank: C-02, C-03, C-04, C-08, C-12. G-20's shape, one level up: a run that did not hold its conditions is DISCARDED, not scaled** |
