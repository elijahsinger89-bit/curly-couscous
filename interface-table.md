...
| S-15 | DOSING: EC probe in the manifold | CONTROL-SOFTWARE: implicit whole-loop check | FROZEN as the only verification that exists. EC rising during a dose confirms the loop moved and that something was delivered. See its three limits in findings.md F-001. Nothing may treat it as per-channel or as valid at rest | closed |
| S-16 | DOSING: pH probe in the manifold | CONTROL-SOFTWARE: attribution of the pH up and pH down channels | OPEN, proposed by DOSING 2026-08-30. F-001 limit 3 says EC does not move for pH up, pH down or fulvic, which is true. A pH probe exists and pH is precisely what those two channels move, so two of the three EC-invisible channels may already be covered by a probe that is bought and read. This extends FROZEN row S-15 and is therefore the owner's decision, not BOSS's and not DOSING's. It costs nothing and adds no hardware | owner decides. The probe step per single dose is a measurement the owner performs, never a datasheet figure |
