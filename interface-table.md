...
| S-19 | Every subsystem that touches a channel: CONTROL-SOFTWARE's channel index, DISPLAY-BOX's logic board output at S-12, INTERCONNECT's cable core, PUMP-BOXES' driver and head in one of two boxes, DOSING's tube, coupling, jug station and jug | one another | OPEN and UNOWNED until now. There is ONE channel token and it must ride the whole chain with no translation at any point. Found by DOSING 2026-08-30 reading the whole table: four subsystems each own a fragment and none owned the token, so it appeared in the table as nothing at all. That is T-002's shape a second time. If the ends disagree, a head labelled N driven by software channel N+1 doses the wrong product every batch, permanently, AND PASSES EVERY CHECK: S-16 attributes the pH movement to the channel that was commanded, and G-05 decrements the wrong jug, so the jug that actually empties does so without warning | BOSS holds the row. The scheme is DOSING's, the carriers belong to PUMP-BOXES, INTERCONNECT and DISPLAY-BOX, the index is CONTROL-SOFTWARE's. Nobody assigns it alone. Verified only by C-09 |
| S-18 | WATER: chiller loop, commanded by MAIN-PANEL's contactor | CONTROL-SOFTWARE: verification windows | OPEN, raised by CONTROL-SOFTWARE 2026-08-30 and explicitly not decided by it. Chiller cycling moves tank temperature, and temperature moves both the pH and the EC readings, so a chiller cycle inside a settle window is a contaminant. Whether the chiller is held off across settle windows is a process trade, temperature excursion against read cleanliness | Owner and WATER decide. CONTROL-SOFTWARE only commands what it is told to |
