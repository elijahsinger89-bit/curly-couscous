...
| S-14 | CONTROL-SOFTWARE: fault registry | the named "no-circulation fault" | OPEN and UNVERIFIED. The fault was named in the project description. What raises it, or whether anything does, is not established. BOSS has not checked any code and is not claiming the fault is absent | CONTROL-SOFTWARE answers from the code, not from memory |
