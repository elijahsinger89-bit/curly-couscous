...
| F-10 | PUMP-BOXES: the head, which the pump tube sits inside | DOSING: the wet path, which the pump tube is part of | **OPEN AND NEW 2026-08-30. Nobody owns the pump tube.** It is PharMed BPT B25, wetted, a consumable at about 1000 h, and it lives inside PUMP-BOXES' head while being part of DOSING's wet path. D-006 gave DOSING the wet path and stopped PUMP-BOXES at the barb, which leaves the tube between them. Third instance of T-002's shape and BOSS did not catch this one either: it surfaced from a manufacturer spec sheet, not from the table | BOSS holds it until DOSING and PUMP-BOXES answer. Whoever owns it also owns the change interval, the change procedure, and telling CONTROL-SOFTWARE that C-01 is void for that channel |
