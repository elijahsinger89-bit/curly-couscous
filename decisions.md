...
**D-160 G-46 PAID FOR ITSELF ON FIRST USE, AND THE ERROR RAN THE FLATTERING WAY.**
D6 now reports **157 filled, 16 no valid value, 7 still empty.** The filled total did
not move when twelve face cells arrived, **because twelve cells that had been
counted as filled were never fillable** - seven duty cells on jackets with no pole,
five face cells on a branch circuit, a wet fitting and three cord caps.

**Under the old two-state count this document would have said 169 of 180 and been
wrong by twelve in the direction that makes a document look more finished than it
is.** That is the whole reason the rule exists and it took one day to prove.

**Sixteen of twenty rows now have no empty cell, up from four. The seven remaining
empties reduce to THREE causes: P-07's form, the leak console's position, and
CBL-07's shape.**

**P-07's form is asked. INTERCONNECT declined to spend face width on spacing and was
right to: the allowance has a prior claim on it from an expected split under F-030,
and committing an allowance before the step that consumes it is known is T-020's
shape.** The cheaper resolution may be that the LINE entry stops being an entry at
all - **D-046's precedent is that receptacles are panel mounted and cords are not
...
**D-185 THE WALL ENVELOPE IS ALREADY ANSWERED AND ONLY THE ARRANGEMENT IS OPEN. M-02
IS TWO QUESTIONS AND ONE OF THEM IS CLOSED.**

**D-090 gives the wall as 8 ft by 8 ft with five measured cable runs and a cut rule
of wall run plus 3 ft.** That is the ENVELOPE. **What M-02 is missing is the
POSITIONS within it** - where the four enclosures, the manifold, the raceway and the
jug stations sit.

**So anything in D1 or D3 waiting on the SIZE is answered today and anything waiting
on the ARRANGEMENT is not.** Being checked against every blocked step in D1, because
**a step blocked on the wrong half of M-02 is a step that could be worked tonight.**
...
**D-195 F-118 CLOSES: THE PANEL IS PLASTIC, SO THE BARE BLOCK BONDS TO NOTHING. AND
THE ANSWER IS RECORDED AS A DEPENDENCY RATHER THAN AS AN OUTCOME.** Owner,
2026-09-23. The Qilisu mounting plate is PC/ABS and the box is polycarbonate.

**So the no-insulators problem disappears, there is no second ground point, and
D-192's single-point rule is not threatened.**

**BUT IT IS RECORDED THE WAY THE OWNER ASKED, WHICH IS THE PART THAT OUTLIVES THIS
BUILD: IF THE PLATE WERE STEEL, THE SAME PART WOULD HAVE CREATED EXACTLY THE SPLIT
D-192 FORBIDS - AND NOBODY WOULD HAVE SEEN IT, BECAUSE THE BLOCK LOOKS IDENTICAL
EITHER WAY.**

**So the rule is not "the block is fine". It is: A BARE GROUND BLOCK'S SAFETY DEPENDS
ON THE PANEL MATERIAL, and the panel material must be checked before the block is
mounted.** A hazard that fails to occur because of a property nobody chose for that
reason **is a lucky build, not a safe one** - and the luck is only visible if the
dependency is written down.

**MOUNTING: DRILL THROUGH, holes at least 5.0 in apart to miss the taps.** The block
is 5.75 in long, so it is achievable at the ends. **Both bars mount the same way,
side by side, with the inter-bar jumper fitted FIRST per C-25.**

**LOOKUP 11 FREES THE FOUR RECEPTACLES FROM THE PLATE ENTIRELY.** Weatherproof boxes
with covers on the wall beside the panel, fed through cord grips. **Nothing about
them is inside the enclosure except the twelve conductors and their grips.**
